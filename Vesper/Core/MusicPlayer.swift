...
@MainActor final class MusicPlayer: ObservableObject {
    ...
    func setQueue(_ values: [JSONValue], append: Bool = false) {
        ...
        initialized = true
        if !tracks.contains(where: { $0.id == track.id }) { pause(); player.replaceCurrentItem(with: nil); track = tracks.first ?? .null; position = 0; duration = 0 }
    }
    ...
    func start(_ value: JSONValue) {
        resolveTask?.cancel(); resolving = false
    ...
    func pause() { resolveTask?.cancel(); resolving = false; player.pause(); playing = false }
